...
describe("prototype pollution guards", () => {
  const POLLUTION_FILENAME = "pollution-test.json";
  const POLLUTION_DEFAULTS = { enabled: true, timeout: 30, label: "default" };

  it("deepMerge skips __proto__ key", () => {
    const malicious = { __proto__: { polluted: true } };
    const result = deepMerge({}, malicious);
    expect((result as Record<string, unknown>).polluted).toBeUndefined();
    expect((Object.prototype as Record<string, unknown>).polluted).toBeUndefined();
    expect({} satisfies Record<string, unknown>).not.toHaveProperty("polluted");
  });

  it("deepMerge skips constructor key", () => {
  ...
  it("deepMerge skips prototype key", () => {
  ...
  it("loadConfig skips __proto__ in project-local file", () => {
    const { globalDir, projectDir, cleanup } = setupDirs("pollution-project");
    writeFileSync(join(globalDir, POLLUTION_FILENAME), JSON.stringify({ label: "global" }));
    writeFileSync(
      join(projectDir, ".pi", POLLUTION_FILENAME),
      JSON.stringify({ __proto__: { polluted: true } }),
    );
    const config = loadConfig(POLLUTION_FILENAME, POLLUTION_DEFAULTS, {
      configDir: globalDir,
      cwd: projectDir,
    });
    expect(config.label).toBe("global");
    expect((Object.prototype as Record<string, unknown>).polluted).toBeUndefined();
    expect({} satisfies Record<string, unknown>).not.toHaveProperty("polluted");
    cleanup();
  });

  it("loadConfig with deep merge skips __proto__ in project-local file", () => {
    const { globalDir, projectDir, cleanup } = setupDirs("pollution-deep");
    const nestedDefaults = { components: { spinner: true } };
    writeFileSync(
      join(globalDir, POLLUTION_FILENAME),
    ...
    const config = loadConfig(POLLUTION_FILENAME, nestedDefaults, {
      configDir: globalDir,
      cwd: projectDir,
      merge: "deep",
    });
    expect(config.components.spinner).toBe(false);
    expect((Object.prototype as Record<string, unknown>).polluted).toBeUndefined();
    cleanup();
  });
});
