/**
 * Preprocessor engine — location-targeted rewrites before schema repair.
 *
 * Kinds: alias (field rename via selector), structural (arbitrary
 * transform at a JSON-pointer location), and typed field preprocessors
 * (filesystem-path, scalar, anchor-bleed, grammar-tokens). Edit-specific
 * preprocessors are assembled in repair/entry.ts; this module resolves
 * selectors (with `*` wildcard) and applies them in order, collecting
 * RepairChanges. Model-gated heuristics (anchor-bleed, grammar-tokens) can
 * optionally degrade to observations instead of mutations per policy.
 */

import { unwrapMarkdownAutoLinks } from "./repair-engine.js";
import type { RepairChange } from "./types.js";

export type ObjectLocationSelector = `/${string}` | "";

export type Preprocessor = AliasPreprocessor | FieldPreprocessor | StructuralPreprocessor;
...
function locationsAt(root: unknown, selector: ObjectLocationSelector): Location[] {
  if (selector === "" || selector === "/") {
    return [{ parent: undefined, key: undefined, value: root }];
  }
  const segments = selector.split("/").slice(1).map(decodePointerSegment);
  let locations: Location[] = [{ parent: undefined, key: undefined, value: root }];
...
function accepts(value: unknown, expected: AliasPreprocessor["accepts"]): boolean {
...
function setLocation(location: Location, value: unknown): boolean {
  ...
  } else if (!Array.isArray(location.parent) && typeof location.key === "string") {
    location.parent[location.key] = value;
  } else {
    return false;
  }
...
function modelMatches(modelId: string | undefined, families?: readonly RegExp[]) {
  ...
  for (const preprocessor of preprocessors) {
    if (preprocessor.kind === "alias") {
      ...
      for (const parentLocation of locationsAt(value, parentSelector)) {
        ...
        const replaceEmpty =
          preprocessor.emptyEquivalentToMissing === true && canonicalValue === "";
        if (
          canonical in record &&
        ...
        for (const alias of preprocessor.aliases) {
          if (!(alias in record) || !accepts(record[alias], preprocessor.accepts)) {
            continue;
          }
    ...
    const heuristic =
      preprocessor.kind === "anchor-bleed" || preprocessor.kind === "grammar-tokens";
    if (heuristic && !modelMatches(modelId, preprocessor.modelFamilies)) continue;

    for (const location of locationsAt(value, preprocessor.selector)) {
      const current = location.value;
      let next: unknown = current;
      if (preprocessor.kind === "filesystem-path" && typeof current === "string") {
        next = unwrapMarkdownAutoLinks(current);
      } else if (preprocessor.kind === "filesystem-path-array" && Array.isArray(current)) {
        next = current.map((item) =>
          typeof item === "string" ? unwrapMarkdownAutoLinks(item) : item,
        );
      } else if (preprocessor.kind === "string-or-array" && typeof current === "string") {
        next = [current];
      ...
      } else if (preprocessor.kind === "anchor-bleed" && typeof current === "string") {
        ...
        if (typeof current === "string") {
          ...
          for (const [key, item] of Object.entries(current)) {
            ...
            record[cleanKey] = typeof item === "string" ? stripGrammarTokens(item) : item;
          }
          next = record;
        }
      }

      if (Object.is(next, current) || JSON.stringify(next) === JSON.stringify(current)) {
        continue;
      }
      const ruleId =
