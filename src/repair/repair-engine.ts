...
import type { EditFieldAliasTable } from "./aliases.js";

/**
 * One mutation rule applied by the repair engine.
 *
 * `apply` mutates `args` in place and returns a model-facing note when
 * it repaired something, or `false` when the rule did not apply.
 */
...
/** Configuration for field aliases, root-string wrapping, and tool-specific structural repairs. */
export interface ToolRepairConfig {
  /**
   * canonical field name -> wrong names models emit for it, matched at any
   * depth by key. Accepts a keyed alias table (preferred: typo-checked
   * canonical fields) or a plain record.
   */
  fieldAliases?: EditFieldAliasTable | Record<string, readonly string[]>;
...
/** Result of a single repair pass over one tool-call input. */
export interface RepairResult {
  outcome: "valid" | "repaired" | "unrepairable";
  /** What repairSchemaInput returns: the untouched input, the repaired input,
   *  or (unrepairable) the untouched input. */
  args: unknown;
  rulesFired: string[];
  notes: string[];
   ...
   * repairSchemaInput matters: handing the raw input back to the caller instead
   * would let Value.Convert corrupt it (null -> "null") and execute the call anyway.
   */
...
/** Raw issue emitted by TypeBox's validator. */
interface RawIssue {
  keyword: string;
  instancePath: string;
...
/** One site in the input where the schema disagreed. */
interface IssueSite {
...
/** Resolve a JSON-pointer path against a root value. */
...
/** True when `value` is an object or array (not null). */
function isContainer(value: unknown): value is Record<string, unknown> | unknown[] {
  return value !== null && typeof value === "object";
}

/** Try to coerce `value` to `schema`; returns whether the converted value is valid. */
function schemaAccepts(schema: TSchema, value: unknown): boolean {
  return Value.Check(schema, value);
}

/** Collect the strict validator's failure sites for targeted repair. */
function collectIssueSites(schema: TSchema, value: unknown): IssueSite[] {
  ...
  for (const issue of collectErrors(schema, value)) {
    ...
    push({ parent, key, keyword: issue.keyword, expected }, `${issue.instancePath}`);
  }
  return sites;
}
...
type IssueRule = (site: IssueSite, toolName: string, config: ToolRepairConfig) => string | false;
...
const renameAliasedField: IssueRule = (site, toolName, config) => {
  ...
  if (key in parent && parent[key] !== undefined && parent[key] !== null) return false;
  const aliases = (config.fieldAliases as Record<string, readonly string[]> | undefined)?.[key];
...
const dropEmptyObjectPlaceholder: IssueRule = (site, toolName) => {
  ...
  if (expected !== "array" || !isPlainObject(parent) || typeof key !== "string") return false;
  const value = parent[key];
...
/** Try to parse a JSON value; returns `undefined` on failure. */
...
/**
 * Re-collect issue sites after each pass that fired a rule, so repairs
 * that reveal nested problems (parse a stringified array, then rename
...
/** FNV-1a hash used to fingerprint failure shapes for local telemetry. */
...
/** Describe one raw issue as a compact `path|keyword|detail` string. */
...
/** Format a human-readable retry message listing the validation failures. */
function formatRetryMessage(toolName: string, issues: RawIssue[], input: unknown): string {
  const lines = issues
    .slice(0, 8)
    .map(
      (issue) =>
...
export function repairSchemaInput(options: {
  ...
  // Unconditional path-field cleanup on a clone. Auto-linked paths are
  ...
  // Strict fast path. No Convert here — see the header comment.
  ...
  // Root repairs — the model sent something other than an object.
  ...
  // Tool-specific structural repairs.
  ...
  // Per-issue repairs at the strict validator's failure sites.
  ...
  for (let pass = 0; pass < MAX_ISSUE_PASSES && !schemaAccepts(schema, current); pass++) {
  ...
  // Final verdict, now through pi's own pipeline (Convert, then
