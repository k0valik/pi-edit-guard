/**
 * Envelope recovery — handles double-encoded JSON strings and truncated
 * object tails before the main repair pipeline runs. Pure, no I/O. Limits
 * (maxInputBytes, nesting depth, decode attempts, candidates, work budget)
 * keep adversarial inputs from stalling the pipeline.
 */

import type { RepairChange, RepairPipelineLimits } from "./types.js";
...
export function recoverEnvelope(
  ...
  if (typeof input === "string" && input.trimStart().startsWith("{")) {
    for (const suffix of TRUNCATED_SUFFIXES) {
      ...
      const parsed = parseJson(escapeJsonStringControlCharacters(`${input}${suffix}`));
